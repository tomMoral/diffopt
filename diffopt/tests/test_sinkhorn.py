...
def sinkhorn_loss(a, b, K, eps, n_iter):
    ...
    z = eps * np.log(np.concatenate([u, v]))
    return dual_loss(z, a, b, K, eps)


def grad_analytic(a, b, K, eps, n_iter):
    u, v = sinkhorn(a, b, K, n_iter)
    return eps * np.log(np.concatenate([u, v]))


grad_autodiff_ = autograd.grad(sinkhorn_loss, argnum=(0, 1))


def grad_autodiff(a, b, K, eps, n_iter):
    return np.concatenate(grad_autodiff_(a, b, K, eps, n_iter))
...
def grad_implicit(a, b, K, eps, n_iter):
    n, m = K.shape
    u, v = sinkhorn(a, b, K, n_iter)
    dx = eps * np.log(np.concatenate([u, v]))
    df = a - u * np.dot(K, v)
    dg = b - v * np.dot(K.T, u)
    bias = np.sum(df) + np.sum(dg)
    df -= bias / (n + m)
    dg -= bias / (n + m)
    H = dzz(K, a, b, np.concatenate([u, v]), eps)
    e, v = np.linalg.eigh(H)
    e_inv = 1 / e
    e_inv[e > -1e-12] = 0.
    H_inv = np.dot(v, v.T * e_inv[:, None])
    dz = np.dot(H_inv, np.concatenate([df, dg]))
    return dx - dz
